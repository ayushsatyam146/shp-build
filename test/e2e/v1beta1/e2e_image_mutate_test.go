...
var _ = Describe("For a Kubernetes cluster with Tekton and build installed", Label("kubernetes", "tekton", "e2e"), func() {
	var (
		err      error
		testID   string
	...
	Context("when a Buildah build with label and annotation is defined", Label("buildah", "annotation", "build"), func() {
		BeforeEach(func() {
	...
	Context("mutate image timestamp", Label("timestamp", "image", "mutation"), func() {
		...
		Context("when using BuildKit based Dockerfile build", Label("buildkit", "dockerfile"), func() {
		...
		Context("when using Buildpacks build", Label("buildpacks", "build"), func() {
			var sampleBuildRun = func(outputTimestamp string) *buildv1beta1.BuildRun {
				return NewBuildRunPrototype().
					Namespace(testBuild.Namespace).
					Name(testID).
		...
		Context("edge cases", Label("edge-cases"), func() {
